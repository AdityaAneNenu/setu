...
class QRSubmission(models.Model):
    ...
    person_photo = models.ImageField(upload_to='qr_submissions/photos/', blank=True, null=True, help_text="Photo of the person submitting")
    phone_number = models.CharField(max_length=15, blank=True, null=True)
    
    # Location data
    village = models.ForeignKey(Village, on_delete=models.CASCADE, null=True, blank=True)
    village_name = models.CharField(max_length=200, help_text="Village name from mobile app")
    ...
    mobile_created_at = models.DateTimeField(null=True, blank=True, help_text="Original creation time on mobile")
    
    # Linked to existing complaint system
    linked_complaint = models.OneToOneField('Complaint', on_delete=models.SET_NULL, null=True, blank=True, related_name='qr_submission')
    linked_gap = models.OneToOneField('Gap', on_delete=models.SET_NULL, null=True, blank=True, related_name='qr_submission')
    ...
    def generate_complaint_from_qr(self):
        ...
        complaint = Complaint.objects.create(
            ...
            village=self.village or Village.objects.filter(name__icontains=self.village_name).first(),
...
class QRComplaintDetail(models.Model):
    ...
    qr_submission = models.OneToOneField(QRSubmission, on_delete=models.CASCADE, related_name='complaint_details')
    ...
    audio_file = models.FileField(upload_to='qr_submissions/audio/', blank=True, null=True)
...
class Gap(models.Model):
    ...
    input_method = models.CharField(max_length=10, choices=INPUT_METHOD_CHOICES, default="image", help_text="Method used to submit this gap")
    audio_file = models.FileField(upload_to='gap_audio/', blank=True, null=True, help_text="Audio file if submitted via voice")
    voice_code = models.CharField(max_length=64, blank=True, null=True, db_index=True, help_text="Unique voice fingerprint for voice-based gaps")
    ...
    expected_completion = models.DateField(blank=True, null=True, help_text="Expected completion date")
    actual_completion = models.DateField(blank=True, null=True, help_text="Actual completion date")
    
    # Budget tracking fields
    budget_allocated = models.DecimalField(max_digits=12, decimal_places=2, blank=True, null=True, help_text="Budget allocated in INR")
    budget_spent = models.DecimalField(max_digits=12, decimal_places=2, default=0, help_text="Budget spent in INR")
...
class SMSStatusUpdate(models.Model):
    ...
    sender_phone = models.CharField(max_length=15, help_text="Phone number that sent SMS")
...
class Complaint(models.Model):
    ...
    pmajay_office = models.ForeignKey(PMAJAYOffice, on_delete=models.CASCADE, null=True, blank=True)
    ...
    # Workflow status
    status = models.CharField(max_length=30, choices=COMPLAINT_STATUS, default='received_post')
    created_at = models.DateTimeField(auto_now_add=True)
    ...
    voice_code = models.CharField(max_length=64, blank=True, null=True, db_index=True, help_text="Unique voice fingerprint for complaint audio")
    recorded_by_agent = models.BooleanField(default=False)
    agent_name = models.CharField(max_length=100, blank=True)
    villager_signature_image = models.ImageField(upload_to='signatures/', blank=True, null=True)
...
class WorkflowLog(models.Model):
    ...
    complaint = models.ForeignKey(Complaint, on_delete=models.CASCADE, related_name='workflow_logs')
...
class SurveyVisit(models.Model):
    ...
    follow_ups_completed = models.JSONField(default=list)  # List of complaint IDs checked
    ...
    audio_recordings = models.JSONField(default=list)  # Audio file paths for illiterate villagers
...
class WorkerContact(models.Model):
    ...
    name = models.CharField(max_length=100)
    phone_number = models.CharField(max_length=15)
    worker_type = models.CharField(max_length=50, help_text="Electrician, Plumber, etc.")
...
class SimpleWorkflowTracker(models.Model):
    ...
    assigned_worker = models.ForeignKey(WorkerContact, on_delete=models.SET_NULL, null=True, blank=True)
    
    current_stage = models.CharField(max_length=30, choices=WORKFLOW_STAGES, default='complaint_received')
    ...
    def send_status_sms(self):
        ...
        message = messages.get(self.current_stage, f"Status update for {self.villager_contact.complaint_number}")
...
class VoiceVerificationLog(models.Model):
    ...
    complaint = models.ForeignKey(Complaint, on_delete=models.CASCADE, related_name='voice_verifications', null=True, blank=True)
    gap = models.ForeignKey('Gap', on_delete=models.CASCADE, related_name='voice_verifications', null=True, blank=True, help_text="Gap being verified")
    verification_audio_path = models.CharField(max_length=500, help_text="Path to verification audio file")
    ...
    verified_by = models.CharField(max_length=100, blank=True, help_text="Person who attempted verification")
    notes = models.TextField(blank=True)
    ...
    verification_voice_code = models.CharField(max_length=64, blank=True, null=True,
    ...
    def __str__(self):
        if self.complaint:
            ...
            return f"Verification - {self.confidence} ({self.similarity_score*100:.1f}%)"
