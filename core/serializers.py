...
class QRSubmissionSerializer(serializers.ModelSerializer):
    ...
    person_photo_base64 = serializers.CharField(write_only=True, required=False, help_text="Base64 encoded photo")
    village_name = serializers.CharField(max_length=200)
    
    class Meta:
        model = QRSubmission
        fields = [
            'submission_uuid', 'qr_code', 'person_name', 'person_photo', 
            'person_photo_base64', 'phone_number', 'village_name', 
            'latitude', 'longitude', 'form_type', 'additional_data',
            'mobile_created_at', 'created_at'
        ]
        ...
        if photo_base64:
            ...
            try:
                ...
                photo_file = ContentFile(
                    ...
                    name=f'qr_photo_{uuid.uuid4()}.{ext}'
                )
...
class QRComplaintDetailSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'qr_submission', 'complaint_text', 'complaint_type', 
            'severity', 'additional_photos', 'audio_file',
            'created_at', 'synced_from_mobile'
        ]
        read_only_fields = ['created_at']
