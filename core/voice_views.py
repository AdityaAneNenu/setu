...
def verify_voice_for_closure(request):
    ...
    try:
        ...
        if verification_result.get('is_match'):
            latest_log = VoiceVerificationLog.objects.filter(
                complaint=complaint
...
def test_voice_comparison(request):
    ...
    try:
        if 'audio1' not in request.FILES or 'audio2' not in request.FILES:
            return JsonResponse({
                'success': False,
                'error': 'Two audio files are required'
            }, status=400)
...
def gap_voice_verification_dashboard(request, gap_id):
    ...
    if not gap.audio_file:
        ...
        return render(request, 'core/voice_verification_dashboard.html', {
            'error': 'No audio file available for this gap',
            'gap': gap
        })
    
    # Get verification history for this gap
    verification_logs = VoiceVerificationLog.objects.filter(
        gap=gap
...
def verify_voice_for_gap_resolution(request):
    ...
    try:
        ...
        result = VoiceVerificationManager.verify_gap_resolution(
            gap_obj=gap,
            verification_audio_file=verification_audio
        )
