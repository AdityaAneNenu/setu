...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for key in retry_keys:
            ...
            self.stdout.write(f"Processing Gap {gap_id} (attempt {attempts}/{max_retries})")
            
            if dry_run:
                self.stdout.write(f"  [DRY RUN] Would retry Firebase sync for Gap {gap_id}")
                continue
                
            # Skip if max retries exceeded
            if attempts > max_retries:
                self.stdout.write(
                    self.style.WARNING(f"  ⚠️ Max retries exceeded for Gap {gap_id}, removing from queue")
                )
                cache.delete(key)
                failed += 1
                continue
            ...
            try:
                ...
                cache.delete(key)
                succeeded += 1
                self.stdout.write(self.style.SUCCESS(f"  ✅ Successfully synced Gap {gap_id}"))
                
            except Gap.DoesNotExist:
                # Gap no longer exists - remove from queue
                cache.delete(key)
                self.stdout.write(self.style.WARNING(f"  ⚠️ Gap {gap_id} not found, removing from queue"))
        ...
        if succeeded > 0:
            self.stdout.write(self.style.SUCCESS(f"✅ Successfully processed {succeeded} items"))
