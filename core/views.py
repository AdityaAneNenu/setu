...
def upload_form(request):
    if request.method == "POST":
        ...
        submission_type = request.POST.get("submission_type", "image")  # "image" or "audio"
            ...
            if result['success']:
                ...
                try:
                    ...
                    data = {
                        ...
                        "translated_text": ai_data.get("translated_text", transcribed_text),
                        ...
                        "recommendations": ai_data.get("recommendations", f"Auto-analyzed from audio in {processor.speech_service.get_language_name(language_code)}")
                    }
                    ...
                    data = {
                        ...
                        "severity": result['priority_level'] if result['priority_level'] in ['low', 'medium', 'high'] else "medium",
                        "recommendations": f"Auto-analyzed from audio complaint in {processor.speech_service.get_language_name(language_code)}"
                    }
                ...
                return JsonResponse({"error": f"Audio processing failed: {result['error']}"}, status=400)
                
        else:
            # Process traditional image submission
            if not image_file:
                return JsonResponse({"error": "Please provide either an image or audio file"}, status=400)
        ...
        gap = Gap.objects.create(
            ...
            audio_file=audio_file if submission_type == "audio" and audio_file else None,
        ...
        if submission_type == "audio" and gap.audio_file:
            try:
                ...
                print(f"✅ Voice code generated for Gap #{gap.id}: {voice_code[:16]}...")
            except Exception as e:
                print(f"⚠️ Warning: Could not generate voice code for Gap #{gap.id}: {e}")
        ...
        if submission_type == 'audio':
            try:
                ...
                language_name = temp_processor.speech_service.get_language_name(language_code)
            except:
...
def analytics(request):
    ...
    for item in gaps_by_type_query:
        ...
        display_percentage = max(round(raw_percentage, 1), 6 if item['count'] > 0 else 0)
    ...
    for item in severity_query:
        percentage = (item['count'] / max_severity_count * 100) if max_severity_count > 0 else 0
...
def update_gap_status(request, gap_id):
    ...
    if request.method == "POST":
        ...
        print(f"🔍 DEBUG: Condition check: new_status='{new_status}' == 'resolved' AND is_voice_gap={is_voice_gap}")
        
        # If trying to resolve a VOICE GAP, voice CODE MATCHING is MANDATORY
        if new_status == 'resolved' and is_voice_gap:
            print(f"🔒 DEBUG: Voice gap resolution detected - VOICE CODE VERIFICATION REQUIRED")
            print(f"🔍 DEBUG: Original gap voice_code: {gap.voice_code[:16] if gap.voice_code else 'None'}...")
            ...
            if not gap.voice_code:
                ...
                if gap.audio_file:
                    try:
                        from .voice_verification import VoiceFeatureExtractor
                        gap.voice_code = VoiceFeatureExtractor.generate_voice_code(gap.audio_file.path)
                        gap.save(update_fields=['voice_code'])
                        print(f"✅ DEBUG: Generated voice code: {gap.voice_code[:16]}...")
                    except Exception as e:
                        print(f"❌ DEBUG: Failed to generate voice code: {e}")
                        messages.error(request, '❌ Cannot verify voice - gap audio processing failed.')
                        return redirect('manage_gaps')
                else:
                    messages.error(request, '❌ Cannot verify voice - no audio file found.')
                    return redirect('manage_gaps')
            ...
            print(f"🔍 DEBUG: Found {verification_count} SUCCESSFUL verification logs (is_match=True)")
            
            if not verified_logs.exists():
                # BLOCK RESOLUTION - No successful voice verification found
                print(f"❌ DEBUG: BLOCKING RESOLUTION - No successful voice verification found!")
                messages.error(
                    request, 
            ...
            print(f"✅ DEBUG: Gap #{gap.id} status updated to {new_status} (voice code verified)")
            messages.success(request, '✅ Gap resolved successfully! Voice codes matched - verified authentic complainant.')
            return redirect('manage_gaps')
        
        # For non-voice gaps or non-resolution status changes
        print(f"ℹ️ DEBUG: Non-voice gap or non-resolution status change - allowing update")
        if new_status in dict(Gap.STATUS_CHOICES):
            ...
            print(f"✅ DEBUG: Gap #{gap.id} status updated to {new_status} (no voice verification required)")
            messages.success(request, f'✅ Gap status updated to {gap.get_status_display()}')
        else:
...
def public_dashboard(request):
    ...
    ongoing_projects = Gap.objects.filter(
        Q(status='open') | Q(status='in_progress')
    ).select_related('village').order_by('-created_at')
    
    # Get completed projects
    completed_projects = Gap.objects.filter(status='resolved').select_related('village').order_by('-actual_completion')[:10]
    ...
    # Budget statistics
    total_budget_allocated = Gap.objects.filter(
        Q(status='open') | Q(status='in_progress')
    ).aggregate(total=Sum('budget_allocated'))['total'] or 0
    
    total_budget_spent = Gap.objects.filter(
        Q(status='open') | Q(status='in_progress')
    ).aggregate(total=Sum('budget_spent'))['total'] or 0
    
    # Projects by type (all gaps for complete chart display)
    projects_by_type = Gap.objects.values('gap_type').annotate(count=Count('id')).order_by('-count')
    
    # Projects by village (only villages with projects for display)
    villages_with_projects = Village.objects.annotate(
        ongoing_count=Count('gap', filter=Q(gap__status__in=['open', 'in_progress'])),
        completed_count=Count('gap', filter=Q(gap__status='resolved'))
    ).filter(ongoing_count__gt=0).order_by('-ongoing_count')
...
def qr_submissions_management(request):
    ...
    # Base queryset
    qr_submissions = QRSubmission.objects.select_related('village', 'linked_gap', 'linked_complaint').order_by('-created_at')
    ...
    if search_query:
        qr_submissions = qr_submissions.filter(
            person_name__icontains=search_query
        ) | qr_submissions.filter(
            qr_code__icontains=search_query
        ) | qr_submissions.filter(
            village_name__icontains=search_query
        )
...
def qr_submission_detail(request, submission_id):
    ...
    qr_submission = get_object_or_404(
        QRSubmission.objects.select_related('village', 'linked_gap', 'linked_complaint'),
        id=submission_id
    )
    ...
    if request.method == 'POST':
        ...
        if action == 'create_gap':
            ...
            gap = Gap.objects.create(
                village=qr_submission.village or Village.objects.filter(
