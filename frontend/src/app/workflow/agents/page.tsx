    ...
    <>
      ...
      <div className={styles.container}>
        ...
        <div className={styles.statsRow}>
          ...
          <div className={styles.statItem}>
            <span className={styles.statValue}>{agents.filter((a) => a.is_active).length}</span>
            ...
            {filteredAgents.map((agent) => (
              <div key={agent.id} className={styles.agentCard}>
                <div className={styles.agentHeader}>
                  <div className={styles.avatar}>{(agent.full_name?.[0] || agent.username[0] || 'A').toUpperCase()}</div>
                ...
                <div className={styles.agentStats}>
                  <div className={styles.agentStat}>
                    <span className={styles.agentStatValue}>{agent.active_complaints || 0}</span>
                    <span className={styles.agentStatLabel}>Active</span>
                  </div>
                  <div className={styles.agentStat}>
                    <span className={styles.agentStatValue}>{agent.resolved_complaints || 0}</span>
                    <span className={styles.agentStatLabel}>Resolved</span>
                  </div>
                  <div className={styles.agentStat}>
                    <span className={styles.agentStatValue}>{agent.assigned_villages?.length || 0}</span>
                  ...
                  <div className={styles.villagesList}>
                    ...
                    <div className={styles.villagesTags}>
                      ...
                      {agent.assigned_villages.length > 3 && (
                        <span className={styles.villageTag}>+{agent.assigned_villages.length - 3} more</span>
                      )}
                    </div>
                  </div>
                )}
